...
# ======================
# 6. MEMBANGUN MODEL EFFICIENTNET-B0
# ======================
print("\n" + "="*60)
...
callbacks = [
    ...
    ModelCheckpoint(
        filepath='models/best_efficientnet_model.h5',
        monitor='val_accuracy',
        save_best_only=True,
        mode='max',
        verbose=1,
...
best_model = tf.keras.models.load_model('models/best_efficientnet_model.h5')
...
def plot_advanced_training_history(history1, history2):
    ...
    plt.suptitle('Advanced Training Analytics', fontsize=16)
    plt.tight_layout()
    plt.show()
...
def plot_enhanced_confusion_matrix(y_true, y_pred, class_names):
    ...
    plt.suptitle('Model Confusion Analysis', fontsize=16)
    plt.tight_layout()
    plt.show()
...
def analyze_class_performance(y_true, y_pred, class_names):
    ...
    plt.suptitle('Class-wise Performance Analysis', fontsize=16)
    plt.tight_layout()
    plt.show()
...
try:
    best_model.save('deteksi_makananV3.keras')
    print("✅ 1. Model saved: deteksi_makananV3.keras")
except Exception as e:
    print(f"❌ Error saving .keras: {e}")

try:
    best_model.save('deteksi_makananV3.h5')
    print("✅ 2. Model saved: deteksi_makananV3.h5")
except Exception as e:
    print(f"❌ Error saving .h5: {e}")

try:
    best_model.save_weights('deteksi_makananV3_weights.h5')
    print("✅ 3. Model weights saved: deteksi_makananV3_weights.h5")
except Exception as e:
...
class_info = {
    ...
    'recall': 0.9050,
    'f1_score': 0.9142,
    'training_date': '2 April 2026'
}

with open('model_info.json', 'w') as f:
    json.dump(class_info, f, indent=4)
print("✅ 4. Model info saved: model_info.json")
...
try:
    ...
    print("✅ 5. Confusion matrix saved: confusion_matrix.npy & .csv")
...
try:
    if 'history_phase1' in locals():
        ...
        print("✅ 6. Training history saved: training_history.json")
...
try:
    ...
    if 'history_phase1' in locals() and hasattr(history_phase1, 'history'):
        axes[0, 0].plot(history_phase1.history.get('accuracy', []), label='Train', linewidth=2)
        if 'val_accuracy' in history_phase1.history:
    ...
    axes[1, 0].barh(class_names, [0.9036] * len(class_names), color='skyblue')  
    axes[1, 0].set_title('Model Accuracy: 91.22%')
    axes[1, 0].set_xlabel('Accuracy')
    axes[1, 0].set_xlim([0, 1])

    axes[1, 1].axis('off')
    axes[1, 1].text(0.1, 0.5, fontsize=12,
                   verticalalignment='center', fontfamily='monospace')

    plt.suptitle('Food Detection Model - Final Results', fontsize=16)
    plt.tight_layout()
    plt.savefig('model_summary.png', dpi=300, bbox_inches='tight')
    print("✅ 7. Model summary saved: model_summary.png")
...
# ======================
# Menghitung jumlah epoch untuk plotting
len_phase1 = len(history_phase1.history['accuracy'])
len_phase2 = len(history_phase2.history['accuracy'])

epochs_phase1 = range(1, len_phase1 + 1)
epochs_phase2 = range(20, 20 + len_phase2)
